...
$btnRun.Add_Click({
	...
	# Log avec timestamp
	$logPath = New-Item -Path "$(Get-Location)_$(Get-Date -Format 'yyyy-MM-dd_HHmm').log" -ItemType File

    # Lancer robocopy
    $args = "`"$src`" `"$dest`" /E /COPY:DAT /DCOPY:T /R:3 /W:5 /V /NP /LOG:`"$logPath`" /XC /XN /XO /XD `"System Volume Information`" `"$RECYCLE.BIN`""
...
$form.Controls.AddRange(@($lblSrc, $txtSrc, $btnSrc, $lblDest, $txtDest, $btnDest, $btnRun))
